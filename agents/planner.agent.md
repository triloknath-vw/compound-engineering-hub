...
**Current plan**: `plans/<feature_name>/plan.md` (workspace-relative path from the active workspace root) - update using #tool:vscode/memory .
...
Save the comprehensive plan document to `plans/<feature_name>/plan.md` via #tool:vscode/memory using a workspace-relative path from the active workspace root (never an external/absolute path), then show the scannable plan to the user for review. You MUST show plan to the user, as the plan file is for persistence only, not a substitute for showing it to the user.

## 4. Refinement

On user input after showing the plan:
- Changes requested → revise and present updated plan. Update `plans/<feature_name>/plan.md` (in the current workspace) to keep the documented plan in sync
